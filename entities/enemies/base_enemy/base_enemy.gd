## BaseEnemy: A base enemy class with health, damage handling, and a health bar.
class_name Enemy
...
func _update_health_bar_appearance() -> void:
	if is_instance_valid(health_bar_node):
		...
		if health_percentage > 0.50:
			current_color = high_health_color
		elif health_percentage > 0.20:
			current_color = medium_health_color
		else:
			current_color = critical_health_color

		health_bar_node.change_healthbar_appearance(current_color)
