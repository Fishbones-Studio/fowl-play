...
func _transition_to_next_state(target_state: WeaponEnums.WeaponState, information: Dictionary = {}) -> void:
	...
	if (current_state.ANIMATION_NAME != null and not current_state.ANIMATION_NAME.is_empty() and weapon and weapon.animation_player and weapon.animation_player.has_animation(current_state.ANIMATION_NAME)):
