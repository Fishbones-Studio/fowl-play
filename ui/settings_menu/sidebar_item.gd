	...
	set(value):
		...
		if value:
			...
			if _was_already_active:
					var jump_tween: Tween = create_tween().set_trans(Tween.TRANS_BOUNCE).set_ease(Tween.EASE_OUT)
					...
					var scale_tween: Tween = TweenManager.create_scale_tween(null, self, Vector2(1.25, 1.25), 0.1)
					TweenManager.create_scale_tween(scale_tween, self, Vector2(1.2, 1.2), 0.3)
		else:
