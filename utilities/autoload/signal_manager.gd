...
## Signal Manager
##
## This script is used to manage signals in the game that are needed in 
## multiple scenes. It is an autoload script, meaning it is loaded automatically
## when the game starts and is accessible from anywhere.
################################################################################
extends Node

# Player signals
signal player_state_transitioned(target_state: PlayerEnums.PlayerStates, info: Dictionary)
signal combat_transition_state(target_state: WeaponEnums.MeleeState, info: Dictionary)
...
# Enemy signals
signal enemy_transition_state(target_state: EnemyEnums.EnemyStates, info: Dictionary)
...
## Ignores the signal if it is called before the cooldown time has passed.
